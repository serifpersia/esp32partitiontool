...
public class UIController implements ActionListener {
	...
	private void attachListeners() {
		...
		for (int i = 0; i < ui.getNumEnableCheckboxes(); i++) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		// Check if the event source is a checkbox
			...
			if (checkbox.isSelected()) {
				ui.getPartitionName(toggleID).setEnabled(true);
				ui.getPartitionType(toggleID).setEnabled(true);
				ui.getPartitionSubType(toggleID).setEnabled(true);
				ui.getPartitionSize(toggleID).setEnabled(true);
				ui.getPartitionOffsets(toggleID).setEnabled(true);
			} else {
				ui.getPartitionName(toggleID).setEnabled(false);
				ui.getPartitionType(toggleID).setEnabled(false);
				ui.getPartitionSubType(toggleID).setEnabled(false);
				ui.getPartitionSize(toggleID).setEnabled(false);
				ui.getPartitionOffsets(toggleID).setEnabled(false);
			}
	...
	private int getToggleID(JCheckBox checkbox) {
		// Iterate through the array of checkboxes to find the index of the given
		// checkbox
		for (int i = 0; i < ui.getNumEnableCheckboxes(); i++) {
